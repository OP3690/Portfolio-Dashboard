...
export async function updateDailyTracking(): Promise<TrackingUpdateResult> {
  ...
  const priceMap = await fetchBulkCurrentPrices(activePredictions.map((p) => p.stockSymbol));
  ...
  for (const prediction of activePredictions) {
    try {
      ...
      const totalReturn = ((currentPrice - prediction.entryPrice) / prediction.entryPrice) * 100;

      const lastEntry = await TrackingEntry.findOne({ predictionId: prediction._id })
      ...
      const entryExpiresAt = new Date(prediction.firstRecommendedDate.getTime() + 60 * 24 * 60 * 60 * 1000);
